...
server.get("/public/usuarios", (req, res) => {
  res.status(200).json(userdb);
});

server.get("/public/usuarios/:id", (req, res) => {
  const { id } = req.params;
  const usuario = userdb.usuarios.find((user) => user.id === parseInt(id));
  if (usuario) {
...
server.delete("/public/deletar", (req, res) => {
  ...
  fs.readFile("./usuarios.json", (err, data) => {
    ...
    const index = json.usuarios.findIndex(
      (user) => user.email === email && user.senha === senha
    );

    json.usuarios.splice(index, 1);
